...
function createDefaultAnnualCashflow() {
    return { income: 0, expenses: 0 };
}

let annualCashflow = createDefaultAnnualCashflow();
let annualReports = [];
...
function addTransaction(label, amount, options = {}) {
    const { affectMonthly = false, affectAnnual = affectMonthly, category = null } = options;
    ...
    if (affectMonthly) {
        if (normalizedAmount >= 0) monthlyCashflow.income = round2(monthlyCashflow.income + normalizedAmount);
        else monthlyCashflow.expenses = round2(monthlyCashflow.expenses + Math.abs(normalizedAmount));
    }
    if (affectAnnual) {
        if (normalizedAmount >= 0) annualCashflow.income = round2(annualCashflow.income + normalizedAmount);
        else annualCashflow.expenses = round2(annualCashflow.expenses + Math.abs(normalizedAmount));
    }
}

function processAnnualTaxAndReport() {
    const year = Math.max(1, Math.floor(elapsedMonths / 12));
    const income = round2(annualCashflow.income);
    const operatingExpenses = round2(annualCashflow.expenses);
    const profitBeforeTax = round2(income - operatingExpenses);
    const tax = profitBeforeTax > 0 ? round2(profitBeforeTax * INCOME_TAX_RATE) : 0;

    if (tax > 0) {
        balance = round2(balance - tax);
        addTransaction(`Daň z příjmů · rok ${year}`, -tax, {
            affectMonthly: true,
            affectAnnual: true,
            category: "Daň z příjmů"
        });
    }

    const totalExpenses = round2(operatingExpenses + tax);
    const netResult = round2(income - totalExpenses);
    const report = { year, income, operatingExpenses, profitBeforeTax, tax, totalExpenses, netResult };
    const existingIndex = annualReports.findIndex(item => item.year === year);
    if (existingIndex >= 0) annualReports[existingIndex] = report;
    else annualReports.push(report);
    annualReports.sort((a, b) => a.year - b.year);

    annualCashflow = createDefaultAnnualCashflow();
    renderAnnualReports();
    addWorldNews("🧾", `Uzávěrka roku ${year}: ${tax > 0 ? `daň ${formatCurrencyInt(tax)}` : "bez daně"}`);
    showGameToast(
        `🧾 Roční uzávěrka · rok ${year}`,
        tax > 0
            ? `Zisk před zdaněním ${formatCurrencyInt(profitBeforeTax)}. Daň 15 % je ${formatCurrencyInt(tax)} a čistý výsledek ${formatCurrencyInt(netResult)}.`
            : `Rok skončil bez zdanitelného zisku. Výsledek je ${formatCurrencyInt(netResult)}, proto se daň neodečetla.`,
        netResult >= 0 ? "lesson" : "negative"
    );
    updateAccount();
}

function normalizeAnnualReport(item) {
    const income = round2(item?.income ?? 0);
    const operatingExpenses = round2(item?.operatingExpenses ?? item?.expenses ?? 0);
    const profitBeforeTax = round2(item?.profitBeforeTax ?? (income - operatingExpenses));
    const tax = Math.max(0, round2(item?.tax ?? 0));
    const totalExpenses = round2(item?.totalExpenses ?? (operatingExpenses + tax));
    return {
        year: Math.max(1, Number(item?.year) || 1),
        income,
        operatingExpenses,
        profitBeforeTax,
        tax,
        totalExpenses,
        netResult: round2(item?.netResult ?? (income - totalExpenses))
    };
}

function renderAnnualReports() {
    const summary = document.getElementById("annualReportSummary");
    const body = document.getElementById("annualReportBody");
    if (!summary || !body) return;

    const currentYear = Math.floor(elapsedMonths / 12) + 1;
    const completedMonths = elapsedMonths % 12;
    const currentProfit = round2(annualCashflow.income - annualCashflow.expenses);
    const estimatedTax = currentProfit > 0 ? round2(currentProfit * INCOME_TAX_RATE) : 0;
    summary.innerHTML = `
        <div><span>Rozpracovaný rok</span><strong>${currentYear}</strong><small>${completedMonths}/12 měsíců</small></div>
        <div><span>Průběžné příjmy</span><strong class="positive">${formatCurrencyInt(annualCashflow.income)}</strong></div>
        <div><span>Průběžné výdaje</span><strong class="negative">${formatCurrencyInt(annualCashflow.expenses)}</strong></div>
        <div><span>Odhad daně</span><strong>${formatCurrencyInt(estimatedTax)}</strong><small>jen při zisku</small></div>
    `;

    if (!annualReports.length) {
        body.innerHTML = '<tr><td colspan="6" class="annual-report-empty">První uzávěrka vznikne po 12 měsících hry.</td></tr>';
        return;
    }

    body.innerHTML = [...annualReports]
        .sort((a, b) => b.year - a.year)
        .map(item => `
            <tr>
                <th scope="row">Rok ${item.year}</th>
                <td class="income-cell">${formatCurrencyInt(item.income)}</td>
                <td>${formatCurrencyInt(item.operatingExpenses)}</td>
                <td>${item.tax > 0 ? formatCurrencyInt(item.tax) : "—"}</td>
                <td>${formatCurrencyInt(item.totalExpenses)}</td>
                <td class="${item.netResult >= 0 ? "income-cell" : "expense-cell"}">${formatCurrencyInt(item.netResult)}</td>
            </tr>
        `).join("");
}
...
function updatePrice() {
    ...
    if (monthTick >= DIVIDEND_PERIOD_TICKS) {
        ...
        processLoanMonth();
        if (elapsedMonths % 12 === 0) processAnnualTaxAndReport();
        renderGameTime();
...
function closeTrade(id, reason = "Manuální uzavření") {
    ...
    balance += settlement;
    const assetName = assets[trade.asset || "growth"]?.name || trade.asset;
    addTransaction(`Uzavření pozice (${assetName})`, settlement);
    if (Math.abs(pnl) >= 0.01) {
        addTransaction(
            `${pnl >= 0 ? "Realizovaný zisk" : "Realizovaná ztráta"} (${assetName})`,
            pnl,
            { affectMonthly: true, category: "Realizované obchody" }
        );
    }
...
function drawPortfolioChart() {
    ...
    transactionHistory.forEach(tx => {
        ...
        const key = tx.category || tx.label || "Neznámé";
...
function buildSaveText() {
    ...
    text += `${JSON.stringify(annualCashflow)}\n\n`;

    text += "=== ANNUAL REPORTS ===\n";
    text += `${JSON.stringify(annualReports)}\n\n`;
...
function parseImportedData(text, options = {}) {
    ...
    /* ----- ANNUAL CASHFLOW ----- */
    let secAnnualCashflow = getSection("ANNUAL CASHFLOW");
    if (secAnnualCashflow) {
        try {
            const parsedAnnual = JSON.parse(secAnnualCashflow.split("\n")[0]);
            if (parsedAnnual && typeof parsedAnnual === "object") {
                annualCashflow = {
                    income: round2(parsedAnnual.income ?? 0),
                    expenses: round2(parsedAnnual.expenses ?? 0)
                };
            }
        } catch {
            annualCashflow = createDefaultAnnualCashflow();
    ...
    /* ----- ANNUAL REPORTS ----- */
    let secAnnualReports = getSection("ANNUAL REPORTS");
    if (secAnnualReports) {
        try {
            const parsedReports = JSON.parse(secAnnualReports);
            if (Array.isArray(parsedReports)) annualReports = parsedReports.map(normalizeAnnualReport);
        } catch {
            annualReports = [];
        }
    }
